...
<script setup>
  ...
  const handleRowClick = (row, column, event) => {
    getDetailList(row.CGNO, row.CLBH, row.XXCC);
  };
  ...
  const handleSetPackQty = (CGNO, CLBH, XXCC, PackQty) => {
  ...
  const handleAddPack = () => {
    ...
    addPackSizeAPI({
      ...
      XXCC: stateDetail.list[0].XXCC,
      USERID: userName.value,
      QTYAD: qtyad.value,
    })
      .then(() => {
        getPurList();
        getDetailList(stateDetail.list[0].CGNO, stateDetail.list[0].CLBH, stateDetail.list[0].XXCC);
        hideLoading();
      })
      .catch(() => {
        hideLoading();
